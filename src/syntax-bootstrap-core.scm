...
(define (cluck-map-ref/default m key default)
  (hash-table-ref/default (map-hash m) key default))

(define (cluck-set-member? s key)
  (hash-table-exists? (set-hash s) key))

(define (cluck-map-insert m key value)
  (begin
    (hash-table-set! (map-hash m) key value)
    m))

(define (cluck-set-insert s key)
  (begin
    (hash-table-set! (set-hash s) key #t)
    s))

(define (cluck-map-delete m key)
  (begin
    (hash-table-delete! (map-hash m) key)
    m))

(define (cluck-set-delete s key)
  (begin
    (hash-table-delete! (set-hash s) key)
    s))
